...
SOURCE = 'narodne-novine.nn.hr (ELI-native)'
...
_FAMILIES: list[dict] = [{'name': 'Official Gazette (Narodne novine)', 'tool': 'hr_list_issue / hr_get_act / hr_get_text'}]

_GAPS: list[dict] = [{'id': 'HR-001', 'family': 'Official Gazette (Narodne novine)', 'missing': 'There is no free-text search - documents are addressed by year, issue and document number.', 'fallback': 'Use the Narodne novine website search to find an issue and document number.'}, {'id': 'HR-002', 'family': 'Official Gazette (Narodne novine)', 'missing': 'The gazette carries acts as published; consolidated (procisceni tekst) versions are a separate publication.', 'fallback': 'Look for the consolidated text on nn.hr where one exists.'}, {'id': 'HR-003', 'family': 'Official Gazette (Narodne novine)', 'missing': 'Legal force on a given date is not evaluated. Text is returned as the source publishes it; this connector does not decide whether a provision is in force.', 'fallback': 'Check the validity or consolidation dates on the source record itself.'}]
