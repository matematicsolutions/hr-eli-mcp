...
Pulls a tiny static ``hr-runtime.json.gz`` from this repo's GitHub Release on
first tool call so a dead endpoint can be repointed (edit runtime/hr-runtime.json
...
Opt-out: set ``HR_ELI_RUNTIME_URL=""`` to disable the fetch.
"""
...
MANIFEST_URL = os.environ.get(
    "HR_ELI_RUNTIME_URL",
    "https://github.com/matematicsolutions/hr-eli-mcp/releases/latest/download/hr-runtime.json.gz",
)
_CACHE_NAME = "hr-runtime.json"
_TIMEOUT = 15
_USER_AGENT = "hr-eli-mcp-runtime (+https://github.com/matematicsolutions/hr-eli-mcp)"

_runtime: dict[str, Any] | None = None


def _cache_dir() -> Path:
    env = os.environ.get("HR_ELI_CACHE_DIR")
    if env:
        return Path(env).expanduser()
    return Path.home() / ".matematic" / "cache" / "hr-eli"
