...
from hr_eli_mcp.coverage import build_coverage
from hr_eli_mcp.server import hr_coverage


def _coverage():
    fn = getattr(hr_coverage, "fn", hr_coverage)
...
def test_tool_writes_an_audit_entry(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HR_ELI_AUDIT_DIR", str(tmp_path))
    _coverage()
    lines = [
        line
    ...
    assert any("hr_coverage" in line for line in lines), (
        "an audit entry was written but it does not name hr_coverage"
    )
